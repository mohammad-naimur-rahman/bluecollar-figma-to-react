...
const StatisticsCard = ({ qty, title }) => {
  return (
    <div className="flex mb-6 last:mb-0 border-2 border-white sm:text-base">
      <div className="bg-primary">
        <p className="py-4 px-6 sm:py-2 sm:px-4 font-bold min-w-[90px] sm:min-w-[70px] text-center">{qty}</p>
      </div>
      <p className="py-4 px-6 sm:py-2 sm:px-4 text-white min-w-[250px] sm:min-w-[180px]">{title}</p>
    </div>
  )
}
  ...
  return (
    <div className="flex bg-white lg:flex-col">
      <div className="w-[45%] relative lg:w-full">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2  px-24 xl:px-12 lg:px-10 sm:px-5 py-40 xl:py-20 lg:py-16 sm:py-8 bg-secondary bg-opacity-70 backdrop-blur-md">
